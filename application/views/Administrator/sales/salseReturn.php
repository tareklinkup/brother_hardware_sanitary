					...
					<table class="table table-bordered">
						...
						<tbody>
							<tr v-for="(product, sl) in cart">
								...
								<td><input type="number" style="width:120px;" v-model="product.return_batra" v-on:input="productReturnTotal(sl)" :readonly="product.return_quantity < 1"></td>
	...
	new Vue({
		...
		methods:{
			...
			productReturnTotal(ind){
				...
				let cartTotal = parseFloat(this.cart[ind].return_quantity) * parseFloat(this.cart[ind].return_rate);
				let batraAmount = this.cart[ind].return_batra ? parseFloat((parseFloat(cartTotal) * parseFloat(this.cart[ind].return_batra)) / 100).toFixed(2) : 0;
				this.cart[ind].return_amount = parseFloat(cartTotal - batraAmount);
				this.calculateTotal();
			},
			calculateTotal(){
			...
			async getReturn() {
				...
				this.cart.map(product => {
					...
					product.return_batra = parseInt((returnDetail?.SaleReturnDetails_BatraAmount * 100) / returnDetail?.SaleReturnDetails_ReturnAmount);
