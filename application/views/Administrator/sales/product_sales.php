	...
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			...
			<div class="form-group" style="display: none;">
				<label class="col-sm-1 control-label no-padding-right no-padding-left"> Reminder Date </label>
				<div class="col-sm-2">
					<input class="form-control" id="reminderDate" type="date" v-model="sales.reminderDate"/>
				</div>
			</div>
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right"> Sale Date </label>
				<div class="col-sm-2">
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
										...
										<td>
											<div class="form-group">
												...
												<div class="col-xs-4 no-padding-right">
													<input type="number" id="vatPercent" class="form-control" min="0" v-model="vatPercent" v-on:input="calculateTotal"/>
												</div>
									...
									<tr style="display: none;">
										<td>
											<div class="form-group">
												...
												<div class="col-xs-12">
													<input type="number" class="form-control" v-model="sales.labourCost" v-on:input="calculateTotal"/>
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-xs-12 control-label no-padding-right">Adjust Amount</label>
												<div class="col-xs-12">
													<input type="number" class="form-control" v-model="sales.adjustAmount" v-on:input="calculateTotal"/>
												</div>
											</div>
										</td>
									</tr>
...
<script>
	...
	new Vue({
		...
		async created(){
			...
			console.log(this.sales.salesDate);
		},
		methods:{
			getEmployees(){
			...
			calculateTotal(){
				...
				this.sales.total = ((parseFloat(this.sales.subTotal) + parseFloat(this.sales.vat) + parseFloat(this.sales.labourCost)  + parseFloat(this.sales.transportCost) + parseFloat(this.sales.adjustAmount)) - parseFloat(this.sales.discount)).toFixed(2);
			...
			async saveSales(){
				...
				// if (this.sales.reminderDate == currentDate && this.selectedCustomer.Customer_Type != 'G' && this.sales.due != 0) {
				// 	alert("You can't sale becuase Due Reminder date is Today!");
				// 	return;
				// }
			...
			async getSales(){
				await axios.post('/get_sales', {salesId: this.sales.salesId}).then(res=>{
					...
					this.sales.reminderDate = sales.Reminder_Date;
					...
					this.sales.adjustAmount = sales.SaleMaster_AdjustAmount;
