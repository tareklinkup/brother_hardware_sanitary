			...
			<div class="table-responsive" id="reportTable">
				<table id="table1" v-if="searchType == 'category' || searchType == 'product'">
                    ...
                    <tbody v-for="data in reportData" style="display:none;" v-bind:style="{display: reportData.length > 0 ? '' : 'none'}">
                        <tr>
                            ...
                            <td style="text-align:right;">{{ data.profit_loss }}</td>
                        </tr>
                    </tbody>
					<tfoot v-bind:style="{display: reportData.length > 0 ? '' : 'none'}">
						<tr>
							...
							<th style="text-align:right;">
								{{ reportData.reduce((prev, curr) => {return prev + parseFloat(curr.purchased_amount)}, 0) }}
							</th>
							<th style="text-align:right;">
								{{ reportData.reduce((prev, curr) => {return prev + parseFloat(curr.SaleDetails_TotalAmount)}, 0) }}
							</th>
							<th style="text-align:right;">
								{{ reportData.reduce((prev, curr) => {return prev + parseFloat(curr.profit_loss)}, 0) }}
							</th>
						</tr>
					</tfoot>
                </table>
				<table id="table1" v-else>
					...
					<tfoot style="font-weight:bold;background-color:#e9dcdc;">
						...
						<tr>
							<td colspan="4" style="text-align:right;">Transport / Labour Cost (-)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ parseFloat(otherIncomeExpense.purchase_transport_cost) }}</td>
						</tr>
