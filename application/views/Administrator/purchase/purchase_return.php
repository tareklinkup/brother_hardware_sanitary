			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr v-for="(product, sl) in cart">
						...
						<td><input style="width:120px;" type="number" v-model="product.return_batra" v-on:input="productReturnTotal(sl)"></td>
			...
			getReturn() {
				axios.post('/get_purchase_returns', { id: this.purchaseReturn.returnId }).then(async res => {
					...
					this.cart.map(product => {
						...
						product.return_batra = parseInt((returnDetail?.PurchaseReturnDetails_BatraAmount * 100) / returnDetail?.PurchaseReturnDetails_ReturnAmount);
