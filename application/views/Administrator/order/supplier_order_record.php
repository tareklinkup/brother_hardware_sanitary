				...
				<table 
					...
					<tbody>
						<tr v-for="purchase in purchases">
							...
							<td style="text-align:center;">
								<a href="" title="Purchase Invoice" v-bind:href="`/supplier_invoice_print/${purchase.PurchaseMaster_SlNo}`" target="_blank"><i class="fa fa-file-text"></i></a>
								<?php if($this->session->userdata('accountType') != 'u'){?>
								<a href="javascript:" title="Edit Purchase" @click="checkReturnAndEdit(purchase)"><i class="fa fa-edit"></i></a>
								<a href="" title="Delivery Supplier Order" @click.prevent="deliveryOrder(purchase.PurchaseMaster_SlNo)"><i class="fa fa-truck"></i></a>
								<a href="" title="Delete Purchase" @click.prevent="deletePurchase(purchase.PurchaseMaster_SlNo)"><i class="fa fa-trash"></i></a>
								<?php }?>
							</td>
						</tr>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_AdjustAmount)}, 0) }}</td>
	...
	new Vue({
		...
		methods: {
			...
			getPurchaseRecord(){
				...
				let url = '/get_supplier_order';
				if(this.recordType == 'with_details'){
					url = '/get_supplier_order_record';
				}
			...
			getPurchaseDetails(){
				...
				axios.post('/get_supplier_orderdetails', filter)
				.then(res => {
					this.purchases = res.data;
				})
				.catch(error => {
			...
			deletePurchase(purchaseId){
				...
				axios.post('/delete_supplier_order', {purchaseId: purchaseId})
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success){
			...
			deliveryOrder(purchaseId){
				...
				axios.post('/delivered_supplier_order', {purchaseId: purchaseId})
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success){
