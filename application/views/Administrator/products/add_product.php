...
<div id="products">
		...
		<div class="row" style="margin-top: 10px;margin-bottom:15px;border-bottom: 1px solid #ccc;padding-bottom: 15px;">
			...
			<div class="col-md-6">
				...
				<div class="form-group clearfix">
					<label class="control-label col-md-4">DP Rate:</label>
					<div class="col-md-7">
						<input type="text" class="form-control" v-model="product.Product_DpRate" required>
					</div>
				</div>
				<div class="form-group clearfix">
		...
		data(){
			return {
				...
				columns: [
                    ...
                    { label: 'DP Price', field: 'Product_DpRate', align: 'center' },
                    { label: 'VAT', field: 'vat', align: 'center' },
                    { label: 'Is Service', field: 'is_service', align: 'center' },
                    { label: 'Quantity', field: 'stock', align: 'center' },
		...
		methods:{
			...
			getProducts(){
				axios.get('/get_products').then(res => {
					...
					this.products.map(p => {
						let stock = this.stocks.find(s => s.product_id == p.Product_SlNo)
						p.stock = stock == undefined ? 0 : stock.current_quantity
						return p;
					})
				})
