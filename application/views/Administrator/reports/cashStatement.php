		...
		<div class="row" style="padding-top:10px;">
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					<thead>
						<tr style="background: #dee4dc;">
							<th colspan="5">Received from Customers</th>
						</tr>
						<tr>
							<th>Invoice</th>
							<th>Date</th>
					...
					<tbody style="display:none;" v-bind:style="{display: receivedFromCustomers.length > 0 ? '' : 'none'}">
						<tr v-for="payment in receivedFromCustomers">
							...
							<td style="text-align:right;">{{ payment.CPayment_discount | decimal }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">
								<span v-if="receivedFromCustomers.length == 0">0.00</span>
								<span style="display:none;" v-bind:style="{display: receivedFromCustomers.length > 0 ? '' : 'none'}">{{ totalDiscountFromCustomers | decimal }}</span>
							</td>
						</tr>
					</tfoot>
				</table>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalDiscountFromCustomers() {
				return this.receivedFromCustomers.reduce((prev, curr) => {
					return prev + parseFloat(curr.CPayment_discount)
				}, 0).toFixed(2);
			},
