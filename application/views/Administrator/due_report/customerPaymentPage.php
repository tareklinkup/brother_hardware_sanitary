			...
			<form @submit.prevent="saveCustomerPayment">
				<div class="row">
					...
					<div class="col-md-5">
						...
						<div class="form-group" style="display: none;" v-bind:style="{ display: payment.CPayment_TransactionType == 'CR' ? '' : 'none' }">
							<label class="col-md-4 control-label">Discount</label>
							<label class="col-md-1">:</label>
							<div class="col-md-3 no-padding-right">
								<input type="number" class="form-control" step="0.01" id="discountPercent" v-model="discountPercent" min="0" v-on:input="calculateDiscount">
							</div>
							<label class="col-md-1">%</label>
							<div class="col-md-3 no-padding-left">
								<input type="number" step="any" class="form-control" v-model="payment.CPayment_discount" v-on:input="calculateDiscount">
							</div>
						</div>
						<div class="form-group">
...
<script>
	...
	new Vue({
		...
		data(){
			return {
				...
				columns: [
                    ...
                    { label: 'Discount', field: 'CPayment_discount', align: 'center' },
		...
		methods:{
			...
			calculateDiscount() {
				if(this.payment.CPayment_amount != '') {
					let amount = this.payment.CPayment_previous_due;
					let total = parseFloat( +this.payment.CPayment_amount + +this.payment.CPayment_discount);
					if(+total > +amount) {
						alert('Payment amount more then Due amount!');
						this.discountPercent = 0;
						this.payment.CPayment_discount = 0;  
						return;
					}
				}
				if(this.payment.CPayment_previous_due == 0) {
