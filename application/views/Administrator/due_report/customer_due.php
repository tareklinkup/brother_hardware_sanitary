...
<div class="row" id="customerDueList">
	...
	<div class="col-md-12" style="display: none" v-bind:style="{display: dues.length > 0 ? '' : 'none'}">
		<div class="row">
			...
			<div class="col-md-2" style="margin: 7px 0px;">
				<label for="comment"><input type="checkbox" value="1" v-model="without_comment" id="comment"> Without Comment</label>
			</div>
		</div>
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th :style="{ display: without_comment == 1 ? 'none' : ''}">Comment</th>
				...
				<tbody>
					<tr v-for="data in dues">
						...
						<td style="text-align: left;">{{ data.owner_name }}</td>
						<td style="text-align: left;">{{ data.Customer_Address }}</td>
						<td>{{ data.Customer_Mobile }}</td>
						<td :style="{ display: without_comment == 1 ? 'none' : ''}">{{ data.comment }}</td>
						<td style="text-align:right">{{ parseFloat(data.dueAmount).toFixed(2) }}</td>
					</tr>
				</tbody>
				<tfoot>
					<tr style="font-weight:bold;">
						<td :colspan="without_comment == 1 ? 5 : 6" style="text-align:right">Total Due</td>
