    ...
    public function addSales(){
        ...
        try{
            ...
            $sales = array(
                ...
                'Reminder_Date' => $data->sales->reminderDate,
                ...
                'SaleMaster_AdjustAmount' => $data->sales->adjustAmount,
        ...
        try{
            ...
            foreach($data->cart as $product){
                $returnDetails = array(
                    ...
                    'SaleReturnDetails_BatraAmount' => ((($product->return_quantity * $product->return_rate) * $product->return_batra) / 100),
...
//    }
    ...
    public function getProCatWiseProfitLoss(){
        $data = json_decode($this->input->raw_input_stream);
        ...
        if(($data->dateFrom != null && $data->dateFrom != '') && ($data->dateTo != null && $data->dateTo != '')){
            $clauses .= " and date(sd.AddTime) between '$data->dateFrom' and '$data->dateTo'";
        }
    ...
    function DueReminderRecord()  {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Customer Due Reminder Record";  
        $data['content'] = $this->load->view('Administrator/sales/reminder_record', $data, TRUE);
        $this->load->view('Administrator/index', $data); 
    }

    public function getDueReminderSales(){
        $data = json_decode($this->input->raw_input_stream);
        $branchId = $this->session->userdata("BRANCHid");

        $clauses = "";
        if(isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != ''){
            $clauses .= " and sm.Reminder_Date between '$data->dateFrom' and '$data->dateTo'";
        }
